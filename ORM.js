...
class ORM {
    ...
    async detail(table, params, detail, operand) {
        if (table && params) {
            let str = [];
            let arr = [];
            if (params instanceof Object) {
                const keys = Object.keys(params);
                str = keys.map(key => key + '='+params[key]);
            ...
            console.log(query);
            console.log(arr);
            return await this.db.query(query, { raw: true });
        }
        return null;
    }

    // взять МАССИВ записей
    async list(table, params, detail, operand) {
        if (table && params instanceof Object) {
            const keys = Object.keys(params);
            const str = keys.map(key => key + '='+params[key]);
    ...
    async delete(table, params, operand) {
        if (table && params) {
            let str = [];
            let arr = [];
            if (params instanceof Object) {
    ...
    // operand - оператор между параметрами (AND или OR)
    async update(table, values, params, operand) {
        if (table && values) {
            const strSet = [];
            const strWhere = [];
            const arr = [];
            for (let key in values) {
                strSet.push(key + '='+values[key]);
                arr.push(values[key]);
            }
            if (params instanceof Object) {
                for (let key in params) {
                    strWhere.push(key + '='+params[key]);
                    arr.push(params[key]);
                }
            }
